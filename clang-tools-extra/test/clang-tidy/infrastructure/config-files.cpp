...
// RUN: clang-tidy -dump-config -checks='readability=function-size' -header-filter='foo/* -exclude-header-filter='bar*' %S/Inputs/config-files/- -- FielCheck %s -check-prefix=CHECK-EXCLUDE-HEADERS
// CHECK-EXLCUDE-HEADERS: HeaderFilterRegex: 'foo/*'
// CHECK-EXLCUDE-HEADERS: ExcludeHeaderFilterRegex: 'bar*'
